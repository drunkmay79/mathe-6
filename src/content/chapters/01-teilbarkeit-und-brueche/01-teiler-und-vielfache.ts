...
/**
 * Тема 1 главы: учебник S. 8-10, тетрадь S. 2-4.
 *
 * Кратные и делители, а с ними ggT и kgV. Дальше в главе они понадобятся,
 * чтобы сокращать дроби и приводить их к общему знаменателю.
 */
export const lesson = defineLesson({
  ...
  theory: [
    ...
    {
      type: 'playground',
      toy: 'vielfache-huepfer',
      caption:
        'Поменяй шаг прыжка и жми «Прыгнуть». Стрелки показывают, куда прыгун приземлился, а внизу собирается то самое множество кратных. Попробуй шаг 5 и шаг 7 — почувствуешь разницу.',
    },
    {
      type: 'visual',
    ...
    },
    {
      ...
      steps: [
        ...
        },
        {
          ...
          explain: 'На 4 и на 5 без остатка не делится — они не делители, просто пропускаем их.',
        },
        {
    ...
    {
      type: 'playground',
      toy: 'teiler-rechteck-bauer',
      caption:
        'Теперь попробуй сама. Возьми 12 квадратиков и меняй, по сколько ставить в ряд. Если ряды сложились ровно — число подходит, это делитель. Если последний ряд неполный, лишние квадратики подсветятся оранжевым. Проверь так все числа от 1 до 12 и найди все делители двенадцати.',
    },
    {
      type: 'visual',
      art: 'kgv-treffen',
    ...
    },
    {
      ...
      toy: 'ggt-kgv-rechner',
      caption:
        'А это сразу оба списка для любой пары чисел. Общие делители и первое общее кратное подсвечиваются зелёным. Поставь 12 и 18 — увидишь ggT = 6 и kgV = 36. Поставь два соседних числа, например 7 и 8, и посмотри, что получится: общий делитель только 1, а kgV — их произведение.',
    },
    {
      type: 'example',
      prompt: {
      ...
      steps: [
        {
          ...
          explain: 'Если картинку рисовать некогда, делают так: выписывают прыжки первого числа.',
        },
        {
          expr: 'V_{6} = \\{6;\\ 12;\\ 18;\\ 24;\\ 30;\\ \\dots\\}',
